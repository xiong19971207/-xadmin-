...
$(function () {
    ...
    $('#xiong').on('click', function () {
        send_sms_code(this, $(''));
        // alert(111);
        var name987612 = $('#xiong').val();
        alert(name987612);
        if ( 3>2) {
           $.ajax({
                type:'POST', // 规定请求的类型（GET 或 POST）
                url:/send_email/, // 请求的url地址
                dataType:'json', //预期的服务器响应的数据类型
                data:{
                    'email':'17855370672@163.com',
                     'code':'17855370672@163.com'
                },//规定要发送到服务器的数据
    ...
    function send_sms_code(sendBtn, tipsId) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $sendBtn.val("发送验证码");
                if (data.email) {
                    Dml.fun.showValidateError($inpRegEmail, data.email);
    ...
    function show_send_sms(time) {
        ...
        if (time <= 0) {
            ...
            $('#1').val("发送验证码").removeAttr("disabled");
            return;
        }
        time--;
